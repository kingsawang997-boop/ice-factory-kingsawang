...
type NumpadState = { isOpen: boolean; type: 'IN' | 'OUT'; product: InventoryProduct | null; value: string; time: string }
...
export default function InventoryCheckPage() {
  ...
  // 🌟 State สลับหมวดหมู่ น้ำแข็ง vs กระสอบ
  const [activeCategory, setActiveCategory] = useState<'main' | 'packaging'>('main')

  const [numpad, setNumpad] = useState<NumpadState>({ isOpen: false, type: 'IN', product: null, value: '0', time: '' })

  const fetchInventory = useCallback(async () => {
    setIsLoading(true)
    const [prodRes, logRes] = await Promise.all([
      // 🌟 ดึงทั้งหมวด main (น้ำแข็ง) และ packaging (กระสอบ)
  ...
  const submitStockChange = async () => {
    ...
    const note = numpad.type === 'IN' 
      ? (product.category === 'packaging' ? 'รับกระสอบเข้าคลัง' : `ผลิตเสร็จ / รับของเข้า (รอบเวลา ${numpad.time} น.)`)
      : (product.category === 'packaging' ? 'เบิกกระสอบไปใช้งาน' : 'เบิกออก / คัดทิ้ง')

    await supabase.from('products').update({ stock: newStock }).eq('id', product.id)
    
    await supabase.from('inventory_logs').insert([{
      id: `LOG-${Date.now()}`, date: new Date().toISOString(), product_id: product.id, product_name: product.name, type: numpad.type, qty: qty, note: note, by: employeeName
    }])
  ...
  // กรองสินค้าตามแท็บ (main/packaging) และคำค้นหา
    ...
    <div className="fixed inset-0 z-[999] flex flex-col bg-slate-100 overflow-hidden text-xs md:text-sm font-sans">
      <div className="p-4 md:p-6 w-full max-w-7xl mx-auto flex flex-col h-full overflow-hidden">
        ...
        <div className="flex gap-4 mt-6 shrink-0">
          ...
          <div className="bg-white border border-slate-200 px-6 rounded-2xl flex items-center gap-3 shadow-sm font-black text-slate-700">
            <span>รวม:</span><span className="text-xl text-blue-600">{currentCategoryStock.toLocaleString()}</span>
          </div>
                ...
                <div key={p.id} className="bg-white p-4 rounded-3xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow flex flex-col h-full relative group">
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-auto">
                    ...
                    <button onClick={() => openNumpad(p, 'OUT')} className="bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 py-3 rounded-xl font-bold transition-transform active:scale-95 flex flex-col items-center justify-center shadow-sm">
                      <span className="text-xl leading-none mb-1">-</span><span className="text-[10px]">เบิกออก</span>
                    </button>
                  </div>
                </div>
              ))}
        ...
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-lg h-[80vh] flex flex-col animate-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50 shrink-0 rounded-t-[2rem]">
              <div><h3 className="font-black text-slate-800 text-xl flex items-center gap-2">⏱️ ประวัติความเคลื่อนไหว</h3><p className="text-[10px] text-slate-500 mt-1 font-bold">บันทึก 20 รายการล่าสุด</p></div>
        ...
        <div className="fixed inset-0 z-[2000] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm print:hidden">
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-sm overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
            <div className={`p-5 border-b flex justify-between items-center ${numpad.type === 'IN' ? 'bg-emerald-50 border-emerald-100' : 'bg-rose-50 border-rose-100'}`}>
              <h3 className={`font-black text-lg flex items-center gap-2 ${numpad.type === 'IN' ? 'text-emerald-700' : 'text-rose-700'}`}>{numpad.type === 'IN' ? '📥 รับเข้าสต๊อก' : '📤 เบิกออกสต๊อก'}</h3>
            ...
            <div className="p-6 space-y-4">
              <div className="text-center space-y-1"><p className="text-xs font-bold text-slate-500">ทำรายการ</p><p className="font-black text-xl text-slate-800 line-clamp-1">{numpad.product.name}</p></div>
              ...
              <button onClick={submitStockChange} className={`w-full text-white font-black py-4 rounded-xl shadow-lg mt-2 text-lg active:scale-95 transition-transform ${numpad.type === 'IN' ? 'bg-emerald-500 hover:bg-emerald-600 shadow-emerald-500/30' : 'bg-rose-500 hover:bg-rose-600 shadow-rose-500/30'}`}>{numpad.type === 'IN' ? '💾 ยืนยันรับเข้าสต๊อก' : '💾 ยืนยันเบิกออกสต๊อก'}</button>
            </div>
          </div>
        </div>
      )}
